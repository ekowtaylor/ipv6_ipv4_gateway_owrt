#!/bin/bash
# Simplified Installation Script for IPv4↔IPv6 Gateway Service (Single Device Mode)
# Installs Python service WITHOUT HTTP API server - uses direct scripts only
#
# Usage:
#   ./install-simple.sh                    # Safe mode (manual steps required)
#   ./install-simple.sh --full-auto        # Do everything automatically
#
...
REQUIRED_FILES=(
    "ipv4_ipv6_gateway_simple.py"
    "gateway_config.py"
    "gateway-status-simple.sh"
)
...
cp ipv4_ipv6_gateway_simple.py "$INSTALL_DIR/ipv4_ipv6_gateway.py"
...
cp gateway-status-simple.sh /usr/bin/gateway-status
chmod +x /usr/bin/gateway-status
echo "✓ Installed: gateway-status"
echo ""
...
echo "      - Use 'gateway-status' command for monitoring"
